...
MALI_BUILD_CMD = \
	$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=$(@D)/mali ARCH=$(KERNEL_ARCH) \
		CROSS_COMPILE=$(TARGET_KERNEL_CROSS) CONFIG_MALI400=m CONFIG_MALI450=m MALI400_UMP=y modules;
MALI_UMP_BUILD_CMD = \
	$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=$(@D)/ump ARCH=$(KERNEL_ARCH) \
		CROSS_COMPILE=$(TARGET_KERNEL_CROSS) CONFIG_UMP=m modules
...
MALI_BUILD_CMD = \
	$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=$(@D)/mali ARCH=$(KERNEL_ARCH) \
                CROSS_COMPILE=$(TARGET_KERNEL_CROSS) CONFIG_MALI400=m CONFIG_MALI450=m CONFIG_DMA_SHARED_BUFFER=y \
                EXTRA_CFLAGS="-DCONFIG_MALI400=m -DCONFIG_MALI450=m" modules

        #debug command
        #$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=$(@D)/mali ARCH=$(KERNEL_ARCH) \
        #       CROSS_COMPILE=$(TARGET_KERNEL_CROSS) CONFIG_MALI400=m CONFIG_MALI400_DEBUG=y CONFIG_MALI450=m CONFIG_DMA_SHARED_BUFFER=y \
        #       EXTRA_CFLAGS="-DCONFIG_MALI400=m -DCONFIG_MALI400_DEBUG=y -DCONFIG_MALI450=m -DDEBUG" modules
else
MALI_BUILD_CMD = \
	$(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(LINUX_DIR) M=$(@D)/mali ARCH=$(KERNEL_ARCH) \
