...
#define MPLAYER_V_SINK      "autovideosink"// "amlvsink" //"sdlvideosink"
#define MPLAYER_A_CONV      "audioconvert"
#define MPLAYER_A_PROC      "audioresample"
#define MPLAYER_A_SINK      "autoaudiosink" //
...
static void mplayer_play (void)
{
    ...
    g_print ("mplayer_play\n");	
}

static void mplayer_ready (void)
{
    g_print ("mplayer_ready in\n");	
    if (!pipeline) return;
    ...
    g_print ("mplayer_ready\n");	
}

static void mplayer_pause (void)
{
           g_print ("mplayer_pause in\n");	
    if (!pipeline) return;
...
char*  update_rewind(char* pURL)
{
 ...
 //   sprintf (urlStr, "%s&play_speed=%f&time_pos=%lu", pURL, play_speed, gCurrentPosition);
    g_print("trickplay=%d,play_speed=%f,time_pos=%lu\n",trickPlay,play_speed,gCurrentPosition);
    return urlStr;
}
...
char*  update_forward(char* pURL)
{
    ...
    sprintf (urlStr, "%s", pURL);
  //  sprintf (urlStr, "%s&play_speed=%f&time_pos=%lu", pURL, play_speed, gCurrentPosition);
    g_print("forward trickplay=%d,play_speed=%f,time_pos=%lu\n",trickPlay,play_speed,gCurrentPosition);
    return urlStr;
}
static int myexit=0;
static gboolean
