...
**************************************************************************/
int extract_adts_header_info(GstAmlAdec *amladec)
{
    adts_header_t hdr;
    GstBuffer *buffer;
    guint8 *p = NULL;
    ...
    if (amladec->codec_data) {
        p = GST_BUFFER_DATA(amladec->codec_data);
...
void adts_add_header(GstAmlAdec *amladec,GstBuffer ** buffer)
{
    gint8 *buf; 
    int size = ADTS_HEADER_SIZE + GST_BUFFER_SIZE(* buffer);   // 13bit valid
    size &= 0x1fff;
    buf = GST_BUFFER_DATA(amladec->codec_data);
...
//Some aac es stream already has adts header,need check the first ADTS_HEADER_SIZE bytes
    ...
    if (buf!=NULL) {
        ...
        if ( GST_BUFFER_SIZE(amladec->codec_data) == ADTS_HEADER_SIZE) {
	      *buffer = gst_buffer_merge(amladec->codec_data,*buffer);		
        }
    }
 #if 0	
...
static int audio_add_header(codec_para_t *apcodec,GstAmlAdec *amladec)
{
    guint ext_size = GST_BUFFER_SIZE(amladec->codec_data);
    unsigned char *extradata = GST_BUFFER_DATA(amladec->codec_data);
    GstBuffer *hdrBuf;
 ...
 GstFlowReturn audiopre_header_feeding(codec_para_t *apcodec,GstAmlAdec *amladec,GstBuffer ** buf)
{
    guint extra_size = 0,ret;
    if (apcodec&&IS_AUIDO_NEED_PREFEED_HEADER(apcodec->audio_type) && apcodec->has_audio)
    {        
        ret = audio_add_header(apcodec,amladec);        
        if (ret != 0) {
            return ret;
        }
    }else if (apcodec&&apcodec->audio_type== AFORMAT_AAC ){
        adts_add_header(amladec, buf);
    }
    return GST_FLOW_OK;
}

static int audioinfo_get_codec_id(int audio_type,GstAmlAdec *amladec)
{

    return 0;
}
int audioinfo_need_set(codec_para_t *apcodec,GstAmlAdec *amladec)
{
  /* if ((apcodec->audio_type == AFORMAT_ADPCM) || (apcodec->audio_type == AFORMAT_ALAC)) {
        apcodec->audio_info.bitrate = amladec->sample_fmt;
    } else if (apcodec->audio_type == AFORMAT_APE) {
        apcodec->audio_info.bitrate = amladec->bits_per_coded_sample;
    } else {
   ...
   // audioinfo_get_codec_id(apcodec->audio_type,amladec);
    ...
    apcodec->audio_info.block_align = amladec->block_align;
    apcodec->audio_info.extradata_size = amladec->codec_data_len;
    if (apcodec->audio_info.extradata_size > 0) {
        ...
        memcpy((char*)apcodec->audio_info.extradata, GST_BUFFER_DATA(amladec->codec_data), apcodec->audio_info.extradata_size);
    }
