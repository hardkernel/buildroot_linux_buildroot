...
/*************************************************************************/
static int h264_add_header(GstAmlVdec *Gstamlvdec,GstBuffer **buffer)
{
    char nal_start_code[] = {0x0, 0x0, 0x0, 0x1};
    int nalsize;
    unsigned char* p;
    int tmpi;
    guint size = GST_BUFFER_SIZE(Gstamlvdec->codec_data);	
    unsigned char* extradata = GST_BUFFER_DATA(Gstamlvdec->codec_data);
    int header_len = 0;
    ...
    if ((p[0] == 0 && p[1] == 0 && p[2] == 0 && p[3] == 1)||(p[0] == 0 && p[1] == 0 && p[2] == 1 ) && size < 1024) {
        g_print("add 264 header in stream before header len=%d", size);
         hdrBuf = gst_buffer_copy(Gstamlvdec->codec_data);
    ...
    hdrBuf = gst_buffer_new_and_alloc(Gstamlvdec->codec_data_len);
...
static int h264_write_header(GstAmlVdec *Gstamlvdec,GstBuffer **buffer)
{    
    h264_add_header(Gstamlvdec, buffer);  
    return 0;
}

/*************************************************************************/
static int m4s2_dx50_mp4v_write_header(GstAmlVdec *Gstamlvdec,codec_para_t *vpcodec)
{
    codec_write(vpcodec,GST_BUFFER_DATA(Gstamlvdec->codec_data),GST_BUFFER_SIZE(Gstamlvdec->codec_data));
    return 0;
}
/*************************************************************************/
static int wmv3_write_header(GstAmlVdec *Gstamlvdec,codec_para_t *vpcodec)
{
    unsigned i, check_sum = 0;
    guint32 data_len = GST_BUFFER_SIZE(Gstamlvdec->codec_data) + 4;
    GstBuffer *hdrBuf=NULL;
    unsigned char *bufout = NULL;  
    ...
    hdrBuf = gst_buffer_merge(hdrBuf,Gstamlvdec->codec_data);	 
...
/*************************************************************************/
static int wvc1_write_header(GstAmlVdec *Gstamlvdec,codec_para_t*vpcodec)
{    
    codec_write(vpcodec,GST_BUFFER_DATA(Gstamlvdec->codec_data)+1,GST_BUFFER_SIZE(Gstamlvdec->codec_data)-1);
    return 0;
}
/*************************************************************************/
 GstFlowReturn videopre_header_feeding(codec_para_t *vpcodec,GstAmlVdec *Gstamlvdec,GstBuffer ** buf)
{ 
    gint ret;
    if (vpcodec&&buf) {
        ...
        if (VFORMAT_MJPEG == vpcodec->video_type) {
            ...
            ret = h264_write_header(Gstamlvdec, buf);
            if (ret != 0) {
                return ret;
            }
        } else if ((VFORMAT_MPEG4 == vpcodec->video_type) &&
                   (VIDEO_DEC_FORMAT_MPEG4_5 == vpcodec->am_sysinfo.format)) {
            ret = m4s2_dx50_mp4v_write_header(Gstamlvdec, vpcodec);
            if (ret !=  0) {
                return ret;
            }
        }
            ...
            ret = wmv3_write_header(Gstamlvdec, vpcodec);
            if (ret != 0) {
                return ret;
            }
        } else if ((CODEC_TAG_WVC1 == avcodec->codec_tag) || (CODEC_TAG_WMVA == avcodec->codec_tag)) {
            ret = wvc1_write_header(Gstamlvdec,vpcodec);
            if (ret != 0) {
                return ret;
            }
        } 
