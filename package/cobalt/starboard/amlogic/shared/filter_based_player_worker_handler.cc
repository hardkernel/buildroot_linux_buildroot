...
#include "filter_based_player_worker_handler.h"
...
bool FilterBasedPlayerWorkerHandler::Init(
  ...
  if (audio_codec_ != kSbMediaAudioCodecNone) {
    ...
    audio_renderer_.reset(new AmlAudioRenderer(audio_codec_, audio_header_, drm_system_));
    if (!audio_renderer_) {
  ...
  if (input_buffer->sample_type() == kSbMediaTypeAudio) {
    ...
    *written = true;
    ...
    if (audio_renderer_->IsEndOfStreamWritten()) {
      ...
      return audio_renderer_->WriteSample(input_buffer, written);
    }
  } else {
    ...
    *written = true;
    ...
    if (video_renderer_->IsEndOfStreamWritten()) {
      ...
      return video_renderer_->WriteSample(input_buffer, written);
    }
  }
...
void FilterBasedPlayerWorkerHandler::Stop() {
  ...
  scoped_ptr<AmlVideoRenderer> video_renderer;
  {
