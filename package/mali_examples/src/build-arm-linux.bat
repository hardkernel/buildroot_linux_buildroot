...
REM Build all the samples for ARM on Windows
...
REM Prepare the list of OpenGL ES 2.0 example directories 
...
REM Prepare the list of OpenGL ES 3.0 example directories 
...
set CMAKE_BUILD_DIRECTORY=build\cmake-files-arm
...
if %IF_RESULT% == true (
    echo Build samples for Linux on ARM. v2.0.0
	echo Usage: build-x86-win32.bat ^<sample_name^>
    echo Set a ^<sample_name^> argument to build one sample for Linux on ARM.
    echo The executable is built inside: build\arm\
    echo.
	...
	echo.
	echo If ^<sample_name^> is empty all examples are built inside: build\arm\
	echo. 
	echo Ensure your are running in a Visual Studio Command Prompt and your PATH contains CMake.
	goto END
)

REM Make sure the CMAKE_BUILD_DIRECTORY directory does not exist.
...
if "%1" == "--release" (
    cmake -G"NMake Makefiles" ..\..\ -DCMAKE_BUILD_TYPE=Release -DTARGET=ARM -DCMAKE_TOOLCHAIN_FILE=..\..\arm-linux.cmake
	REM Move on to the next argument now ignoring the first one
	shift
) else (
    cmake -G"NMake Makefiles" ..\..\ -DTARGET=ARM -DCMAKE_TOOLCHAIN_FILE=..\..\arm-linux.cmake
)
...
if NOT "%1" == "" ( 
    ...
    echo.
    echo Sample %1 not found. Run 'build-arm-linux.bat --help' to see list of available samples.
    echo.
) else (
...
REM Jump back to the top level directory
cd %ROOTDIRECTORY%
