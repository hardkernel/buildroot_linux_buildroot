...
static void Prepare_CBR_BitsTable(gx_fast_enc_drv_t* p, bool rc)
{
    ...
    if (p->logtime && rc)
        LOGAPI("Prepare_CBR_BitsTable: want:%d, real:%d, diff:%d \n", size, total_bits, size - total_bits);
}
...
static AMVEnc_Status start_ime_cbr(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        LOGAPI("start_ime_cbr: poll fail, fd:%d \n", p->fd);
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            LOGAPI("start_ime_cbr: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_ime_cbr: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            LOGAPI("start_ime_cbr: need time: %d us, base qp: %d, frame num:%d, fd:%d \n", total_time, p->quant, p->total_encode_frame, p->fd);
...
static AMVEnc_Status start_intra_cbr_twice(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        LOGAPI("start_intra_cbr_twice: poll fail, fd:%d \n", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            LOGAPI("start_intra_cbr_twice: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_intra_cbr_twice: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            p->total_encode_time += total_time;
            LOGAPI("start_intra_cbr_twice: need time: %d us, base qp: %d, frame num:%d, fd:%d \n", total_time, p->quant, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}
...
static AMVEnc_Status start_intra_cbr(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        LOGAPI("start_intra_cbr: poll fail, fd:%d \n", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            LOGAPI("start_intra_cbr: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_intra_cbr: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            LOGAPI("start_intra_cbr: need time: %d us, base qp: %d, frame num:%d, fd:%d \n", total_time, p->quant, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}
...
static AMVEnc_Status start_ime_one_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        LOGAPI("start_ime_one_pass: poll fail, fd:%d \n", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_PICTURE_READY;
            LOGAPI("start_ime_one_pass: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_ime_one_pass: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            LOGAPI("start_intra_one_pass: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_intra_one_pass: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
...
static AMVEnc_Status start_ime_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_PICTURE_READY;
            LOGAPI("start_ime_two_pass: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_ime_two_pass: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if (encode_poll(p->fd, -1) <= 0) {
            LOGAPI("start_ime_two_pass: poll fail, fd:%d \n", p->fd);
            return AMVENC_TIMEOUT;
        }
...
static AMVEnc_Status start_intra_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        LOGAPI("start_intra_two_pass: poll fail, fd:%d \n", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            LOGAPI("start_intra_two_pass: done size: %d, fd:%d \n", result[0], p->fd);
        }
    } else {
        LOGAPI("start_intra_two_pass: encode timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
...
void* GxInitFastEncode(int fd, amvenc_initpara_t* init_para) {
    ...
    if (!init_para) {
        LOGAPI("InitFastEncode init para error.  fd:%d \n", fd);
        return NULL;
    }

    p = (gx_fast_enc_drv_t*) calloc(1, sizeof(gx_fast_enc_drv_t));
    if (!p) {
        LOGAPI("InitFastEncode calloc faill. fd:%d \n", fd);
        return NULL;
    }
    ...
    p->fd = fd;
    if (p->fd < 0) {
        LOGAPI("InitFastEncode open encode device fail, fd:%d \n", p->fd);
        free(p);
        return NULL;
    }
    ...
    if (p->mmap_buff.addr == MAP_FAILED) {
        LOGAPI("InitFastEncode mmap fail, fd:%d \n", p->fd);
        free(p);
        return NULL;
    }
    ...
    if (ret) {
        LOGAPI("InitFastEncode config init fai, fd:%dl \n", p->fd);
    ...
    if (p->mb_info == NULL) {
        LOGAPI("ALLOC mb info memory failed. fd:%d \n", p->fd);
        munmap(p->mmap_buff.addr, p->mmap_buff.size);
        free(p);
        return NULL;
    }
...
AMVEnc_Status GxFastEncodeInitFrame(void *dev, ulong *yuv, AMVEncBufferType type, AMVEncFrameFmt fmt, bool IDRframe) {
    ...
    if (p->logtime) {
        ...
        total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
        p->total_encode_time += total_time;
        LOGAPI("GxVEncodeInitFrame: need time: %d us, ret:%d, fd:%d \n", total_time, ret, p->fd);
    }
    return ret;
}
...
AMVEnc_Status GxFastEncodeSPS_PPS(void* dev, unsigned char* outptr, int* datalen) {
    ...
    LOGAPI("FastEncodeSPS_PPS status:%d, fd:%d \n", status, p->fd);
    ret = AMVENC_FAIL;
    if (status == ENCODER_PICTURE_DONE) {
        ...
        LOGAPI("sps pps timeout, status:%d, fd:%d \n", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    return ret;
}
...
void GxUnInitFastEncode(void* dev) {
    ...
    if (p->logtime)
        LOGAPI("total_encode_frame: %d, total_encode_time: %d ms, fd:%d \n", p->total_encode_frame, p->total_encode_time / 1000, p->fd);
    free(p);
    return;
}
