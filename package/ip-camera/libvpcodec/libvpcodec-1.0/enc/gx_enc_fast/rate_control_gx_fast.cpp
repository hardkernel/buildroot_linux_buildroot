...
static void updateRateControl(GxFastEncRateControl *rateCtrl, bool IDR) {
    ...
    //LOGAPI("new: buffer_fullness:%lld numFrameBits:%d \n", rateCtrl->buffer_fullness, rateCtrl->Rc);
    ...
    if (rateCtrl->rcEnable == true) {
        ...
        if (rateCtrl->encoded_frames == 0) {
            ...
            if (rateCtrl->frame_rate != frame_rate || rateCtrl->bitRate != bitrate || rateCtrl->force_IDR) {
                ...
                LOGAPI("we got new config, frame_rate:%f, bitrate:%d, force_IDR:%d rateCtrl->frame_rate%d \n", frame_rate, bitrate, rateCtrl->force_IDR, rateCtrl->frame_rate);
            } else {
                rateCtrl->refresh = false;
            }
            ...
            if (rateCtrl->refresh) {
                ...
                //LOGAPI("new: 1 buffer_fullness:%lld frame_rate:%f estimate_fps:%f \n", rateCtrl->buffer_fullness, rateCtrl->frame_rate, rateCtrl->estimate_fps);
            } else {
