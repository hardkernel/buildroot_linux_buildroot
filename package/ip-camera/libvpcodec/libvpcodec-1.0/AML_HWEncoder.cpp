    ...
    if (info->first_frame) {
        ...
        if (info->frame_in_gop >= (uint) info->idrPeriod && info->idrPeriod > 0) { /* first frame or IDR*/
            ...
            if (force_IDR) {  // todo: force_IDR only need one IDR or reset gop.
                LOGAPI("force next frame to idr :%d, handle: %p. \n", force_IDR, Handle);
                info->nal_unit_type = AVC_NALTYPE_IDR;
                info->slice_type = AVC_I_SLICE;
            } else {
...
AMVEnc_Status AML_HWEncInitialize(AMVEncHandle *Handle, AMVEncParams *encParam, bool* has_mix, int force_mode) {
    ...
    *has_mix = (info->hw_info.dev_id == M8) ? true : false;
    LOGAPI("AML_HWEncInitialize success, handle: %p, fd:%d \n", Handle, info->hw_info.dev_fd);
    return AMVENC_SUCCESS;
exit:
    if (info) {
    ...
    LOGAPI("AML_HWEncInitialize Fail, error=%d. handle: %p \n", status, Handle);
    return status;
}
...
AMVEnc_Status AML_HWSetInput(AMVEncHandle *Handle, AMVEncFrameIO *input) {
    ...
    if (info == NULL) {
        LOGAPI("AML_HWSetInput Fail: UNINITIALIZED. handle: %p \n", Handle);
    ...
    if (info->state == AMVEnc_WaitingForBuffer) {
        ...
        LOGAPI("AML_HWSetInput Wrong state: %d. handle: %p \n", info->state, Handle);
        return AMVENC_FAIL;
    }
    if (input->pitch > 0xFFFF) {
        LOGAPI("AML_HWSetInput Fail: NOT_SUPPORTED. handle: %p \n", Handle);
    ...
    if (AMVENC_SUCCESS != DetermineFrameNum(Handle, info, input->coding_timestamp, (uint32) input->frame_rate, (input->op_flag & AMVEncFrameIO_FORCE_IDR_FLAG))) {
        LOGAPI("AML_HWSetInput SKIPPED_PICTURE, handle: %p \n", Handle);
    ...
    LOGAPI("AML_HWSetInput status: %d. handle: %p \n", status, Handle);
    return status;
}
...
AMVEnc_Status AML_HWEncNAL(AMVEncHandle *Handle, unsigned char *buffer, unsigned int *buf_nal_size, int *nal_type) {
    ...
    if (info == NULL) {
        LOGAPI("AML_HWEncNAL Fail: UNINITIALIZED. handle: %p \n", Handle);
        return AMVENC_UNINITIALIZED;
    }

    LOGAPI("AML_HWEncNAL state: %d. handle: %p \n", info->state, Handle);
    switch (info->state) {
        ...
        case AMVEnc_Encoding_SPS:
            ...
            if (ret != AMVENC_SUCCESS) {
                LOGAPI("AML_HWEncNAL state: %d, err=%d, handle: %p \n", info->state, ret, Handle);
                return ret;
            }
            if (info->outOfBandParamSet)
                info->state = AMVEnc_Analyzing_Frame;
        ...
        case AMVEnc_Encoding_Frame:
            if (*nal_type == 0xff) {
                LOGAPI("AML_HWEncNAL Cancel Current Frame. handle: %p \n", Handle);
                info->state = AMVEnc_Analyzing_Frame;
                ret = AMVENC_SUCCESS;
                return ret;
            }
    ...
    LOGAPI("AML_HWEncNAL next state: %d, ret=%d. handle: %p \n", info->state, ret, Handle);
    return ret;
}
...
AMVEnc_Status AML_HWEncRelease(AMVEncHandle *Handle) {
    ...
    Handle->object = NULL;
    LOGAPI("AML_HWEncRelease Done, handle: %p \n", Handle);
    return AMVENC_SUCCESS;
}
