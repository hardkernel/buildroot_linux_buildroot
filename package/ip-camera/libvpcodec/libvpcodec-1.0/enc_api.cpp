...
AMVEnc_Status AMInitRateControlModule(amvenc_hw_t *hw_info)
{
    ...
    if (!hw_info->rc_data) {
        LOGAPI("AMInitRateControlModule Fail, dev type:%d. fd:%d \n",hw_info->dev_id, hw_info->dev_fd);
        return AMVENC_FAIL;
    }
...
//    iret = ioctl(fd, AMVENC_AVC_IOC_GET_DEVINFO,&dev_info[0]);
    ...
    if ((hw_info->dev_id <= NO_DEFINE) || (hw_info->dev_id >= MAX_DEV)) {
        LOGAPI("Not found available hw encoder device, fd:%d \n", fd);
        close(fd);
        return AMVENC_FAIL;
    }
    ...
    if (!hw_info->dev_data)
    {
        LOGAPI("InitAMVEncode Fail, dev type:%d. fd:%d \n", hw_info->dev_id, fd);
